    ...
    return struct {
        ...
        fn emit(ptr: *anyopaque, record: *const Record) void {
            ...
            if (!format_ok and !truncated) {
                // Format error (not truncation) — discard partial output, emit fallback.
            ...
            if (truncated) {
                // Emit a valid JSON fallback instead of malformed partial output.
                // Downstream parsers (fluentd, vector, loki) need valid JSON on every line.
        ...
        fn formatRecord(w: std.io.AnyWriter, record: *const Record) !void {
            ...
            try writeJsonEscaped(w, record.message);
            try w.writeByte('"');
            ...
            for (record.fields) |f| {
                ...
                try w.writeAll("\":");
                try f.value.formatJson(w);
            }

            if (record.src) |src| {
                try w.writeAll(",\"src\":\"");
                try writeJsonEscaped(w, src.file);
        ...
        const writeJsonEscaped = Field.writeJsonEscaped;
    };
}
...
test "json handler output" {
    ...
    try testing.expectEqualStrings("{\"level\":\"info\",\"time\":0.000000000,\"msg\":\"hello\",\"port\":8080,\"env\":\"prod\"}\n", output);
}

test "json handler negative sub-second timestamp" {
    const testing = std.testing;
    var buf: [512]u8 = undefined;
    var fbs = std.io.fixedBufferStream(&buf);
    var writer = fbs.writer();
    ...
    const record: Record = .{
        ...
        .message = "neg",
        .timestamp_ns = -500_000_000, // -0.5s
        .fields = &.{},
    };
    h.handler().emit(&record);

    const output = fbs.getWritten();
    try testing.expect(std.mem.indexOf(u8, output, "\"time\":-0.500000000") != null);
}

test "json handler rfc3339 timestamp" {
    const testing = std.testing;
    var buf: [512]u8 = undefined;
    var fbs = std.io.fixedBufferStream(&buf);
    var writer = fbs.writer();
    ...
    const output = fbs.getWritten();
    try testing.expect(std.mem.indexOf(u8, output, "\"time\":\"2025-02-20T14:40:45.123456789Z\"") != null);
}

test "json handler custom key names" {
    const testing = std.testing;
    var buf: [512]u8 = undefined;
    var fbs = std.io.fixedBufferStream(&buf);
    var writer = fbs.writer();
    ...
    const output = fbs.getWritten();
    try testing.expect(std.mem.indexOf(u8, output, "\"severity\":\"info\"") != null);
    try testing.expect(std.mem.indexOf(u8, output, "\"timestamp\":") != null);
    try testing.expect(std.mem.indexOf(u8, output, "\"message\":\"hello\"") != null);
    // Default keys must not appear
    try testing.expect(std.mem.indexOf(u8, output, "\"level\"") == null);
    try testing.expect(std.mem.indexOf(u8, output, "\"time\"") == null);
    try testing.expect(std.mem.indexOf(u8, output, "\"msg\"") == null);
}
